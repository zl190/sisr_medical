...
class MySRResNet():
  ...
  def __call__(self):
    ...
    for i in range(int(math.log(self.upsampling_rate,2))):
      # conv4_1, k3n256s1
      x = tf.keras.layers.Conv2D(64, 3, strides=(1,1), padding='same', name='conv0_{}'.format(i+1))(x) 
      x = tf.nn.space_to_depth(x, block_size=2)               
      x = tf.keras.layers.PReLU(alpha_initializer='zeros')(x)

      
    x1 = tf.keras.layers.Conv2D(64, 3, padding='same',name='conv1')(x)
    ...
    for i in range(int(math.log(self.upsampling_rate,2))):
      # conv4_1, k3n256s1
      x = tf.keras.layers.Conv2D(256, 3, strides=(1,1), padding='same', name='conv4_{}'.format(i+1))(x) 
